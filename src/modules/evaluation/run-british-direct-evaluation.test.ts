/**
 * British direct-evaluation orchestration integration test baseline.
 *
 * Tests the composition sequence and result passthrough of
 * runBritishDirectEvaluation without hitting real preparation,
...
vi.mock("@/modules/qualification/prepare-british-direct-evaluation", () => ({
  prepareBritishDirectEvaluation: vi.fn(),
}));
...
import { runBritishDirectEvaluation } from "./run-british-direct-evaluation";
...
const mockPrepare = vi.mocked(prepareBritishDirectEvaluation);
...
const MOCK_PREPARED = { __tag: "prepared" } as never;
...
describe("runBritishDirectEvaluation", () => {
  beforeEach(() => {
  ...
  it("calls preparation with the input params", async () => {
    ...
    await runBritishDirectEvaluation(PARAMS);

    expect(mockPrepare).toHaveBeenCalledWith(PARAMS);
  });

  it("passes prepared output to rule-context resolver", async () => {
    setupSuccessMocks();
  ...
  it("returns composed result with all expected fields", async () => {
    ...
    const result = await runBritishDirectEvaluation(PARAMS);
  ...
  it("rethrows when preparation throws", async () => {
    ...
    await expect(runBritishDirectEvaluation(PARAMS)).rejects.toThrow(err);
