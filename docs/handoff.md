...
Milestone 2C, 2C.1, and 2C.2 (route-level Direct Evaluation smoke and its two coverage extensions) are complete and accepted. Commit `bc4ea62` (2C) added one local-only script `scripts/smoke/direct-evaluation-route-smoke.ts` that exercises the real `POST /api/direct-evaluation` route on a running local Next dev server against the Milestone 2B seed, signing the seeded advisor in via `@supabase/supabase-js` and using `@supabase/ssr` `createServerClient` + `setSession` with an in-memory cookie adapter so the library writes the canonical Supabase auth cookies (no hand-crafted cookie names) which the smoke forwards on `fetch()`; local-only guards refuse `NODE_ENV=production` and refuse unless both `SUPABASE_URL` and `NEXT_BASE_URL` hostnames are `127.0.0.1`/`localhost`. Commit `59737e0` (2C.1) added one root `student_profiles` row to the Milestone 2B seed (no `student_profile_answers`, no `student_profile_subjects`) and a matching-org `sourceProfileId` happy-path assertion that confirms 200 eligible with `persistedRuleTraceCount === 4`; `sourceProfileId` is used only as ownership/persistence reference, not as evaluation source content (the runtime always evaluates the request payload). Commit `ac35feb` (2C.2) added a role-based 403 assertion: with the seeded `advisor` membership, sending `evaluation.allowedRoles = ["owner"]` exercises `requireActorAccess` → `requireOrgRole` end-to-end through the real route surface and produces HTTP 403 with `error.code === "access_denied"` (the route's classifier maps the `Insufficient role:` exception accordingly). Final route-smoke surface: A happy authenticated POST with `sourceProfileId: null` → 200 eligible; B unauthenticated → 401 `authentication_required`; C invalid JSON → 400 `invalid_json`; D invalid request shape → 400 `invalid_request_shape`; E random/nonexistent `sourceProfileId` → 403 `access_denied` (unified outward); F matching-org `sourceProfileId` → 200 eligible; G `allowedRoles=["owner"]` → 403 `access_denied`. Validation across all three slices: `npx supabase db reset` (all 17 migrations); seed PASS 23/23 → 24/24 (after 2C.1's one extra row); seed idempotent re-run; route smoke PASS 8/8 → 9/9 → 10/10 (3 setup + assertions A–G); `npm test` 307/307 across 20 test files; `npm run typecheck` clean. Cross-org `sourceProfileId` 403 and multi-active-membership 409 `org_selection_required` remain future and not selected. RLS isolation continues to be covered by Milestone 1 RLS migrations (00010–00017), not by the route smoke. Dev-server lifecycle is operator-managed (the smoke does not spawn `npm run dev`). Auto-touched framework files (`next-env.d.ts`, `tsconfig.json`) produced by Next 15 dev startup are not committed by these slices; standing operator practice is to revert before push. No business UI, no admin UI, no import pipeline, no CRM features, no General Comparison work exists.
...
### Milestone 2C / 2C.1 / 2C.2 Direct Evaluation route-level smoke

- `scripts/smoke/direct-evaluation-route-smoke.ts` (added in commit `bc4ea62`, extended in commits `59737e0` and `ac35feb`) — local-only TypeScript smoke that exercises the real `POST /api/direct-evaluation` route on a running local Next dev server against the Milestone 2B seed. Local-only guards: refuses `NODE_ENV=production`; refuses unless both `SUPABASE_URL` and `NEXT_BASE_URL` hostnames (parsed via `URL.hostname`) are `127.0.0.1` or `localhost`; never logs the anon key or auth tokens. Auth flow: signs the seeded advisor in via `@supabase/supabase-js` `signInWithPassword`; constructs an in-memory `cookies` adapter and uses `@supabase/ssr` `createServerClient` + `setSession` so the library writes the canonical Supabase auth cookies into the adapter (no hand-crafted cookie names); forwards the captured cookies as a single `Cookie:` header on `fetch()` calls. Reachability gate: GETs the route URL on startup; if the dev server is unreachable, prints `Start the Next dev server first: npm run dev` and exits 1 (the smoke does not spawn the dev server). Final assertion surface (10/10 — 3 setup + 7 assertions): A happy authenticated POST with `sourceProfileId: null` → 200, `runtime.assembled.finalStatus === "eligible"`, non-empty `evaluationRunId` and `evaluationResultId`, `persistedRuleTraceCount === 4`; B unauthenticated POST → 401 `authentication_required`; C invalid JSON body → 400 `invalid_json`; D valid JSON missing `british_curriculum` payload → 400 `invalid_request_shape`; E authenticated POST with random/non-existent `sourceProfileId` UUID → 403 `access_denied` (unified outward source-profile failure); F (Milestone 2C.1) authenticated POST with `sourceProfileId` = the seeded same-org `student_profiles` row id → 200 eligible with 4 traces persisted (the seeded row was added by extending `scripts/seed/demo-seed.ts`'s `seedSampleStudentProfile` step — a single root `student_profiles` row, no `student_profile_answers`, no `student_profile_subjects`); G (Milestone 2C.2) authenticated POST with `evaluation.allowedRoles = ["owner"]` against the seeded `advisor` role → 403 `access_denied` (exercises `requireActorAccess` → `requireOrgRole` end-to-end through the real route surface; the assertion pins only the typed error code, not the internal exception text). Documented intentional limitations: cross-org `sourceProfileId`, multi-active-membership 409 `org_selection_required`, and RLS isolation are not validated; RLS coverage remains in Milestone 1 RLS migrations 00010–00017. Dev-server lifecycle is operator-managed; auto-touched framework files (`next-env.d.ts`, `tsconfig.json`) from Next 15 dev startup are not part of these commits.
...
- **Route-level smoke** — **complete** (Decision 084, commit `bc4ea62`). `scripts/smoke/direct-evaluation-route-smoke.ts` exercises the real `POST /api/direct-evaluation` route against a running local Next dev server, validating HTTP transport, the Supabase cookie/session auth boundary (real cookies via `@supabase/ssr`), `requireActorAccess`, route error classification (400 `invalid_json`, 400 `invalid_request_shape`, 401 `authentication_required`, 403 `access_denied` for the `sourceProfileId` boundary failure), and happy-path persistence through `persist_direct_evaluation_run_atomic`.
- **Matching-org sourceProfileId route smoke** — **complete** (Decision 085, commit `59737e0`). The seed adds exactly one root `student_profiles` row scoped to the seeded organization and owned by the seeded advisor (no `student_profile_answers`, no `student_profile_subjects`); the route smoke's assertion F sends an authenticated POST with that `sourceProfileId` and verifies HTTP 200, `finalStatus === "eligible"`, and `persistedRuleTraceCount === 4`. `sourceProfileId` is used purely as ownership/persistence reference, not as evaluation source content.
- **Role-based 403 route smoke** — **complete** (Decision 086, commit `ac35feb`). The route smoke's assertion G sends an authenticated POST with `evaluation.allowedRoles = ["owner"]` against the seeded `advisor` role and verifies HTTP 403 with `error.code === "access_denied"`. The role guard fires inside `requireActorAccess` → `requireOrgRole` before the workflow runs, so assertion G persists no rows.
- **Cross-org sourceProfileId route smoke** (future candidate, not currently selected). Would require a second organization plus a `student_profiles` row in it, so the seeded advisor's authenticated POST referencing that foreign-org `sourceProfileId` is rejected with 403 `access_denied` (cross-org branch of the same unified outward message). Currently uncovered.
- **Multi-active-membership 409 route smoke** (future candidate, not currently selected). Would require a second `organization_memberships` row for the seeded advisor in a second organization so `resolveOrgContext` returns `multiple_active_memberships_requires_selection`. A request without `evaluation.organizationId` would then surface 409 `org_selection_required`. Currently uncovered.
- **Milestone 2D candidate — resolver/runtime dependency injection** (future, not currently selected). The runtime rule/context resolution path is currently coupled to `next/headers.cookies()` via `@/lib/supabase/server.createClient`. This blocks standalone tsx scripts from invoking the user-facing `runAndPersistDirectEvaluation` workflow — Milestone 2B's smoke composes the pure runtime modules and replicates the workflow's persistence mapping inline as a result. The Milestone 2C/2C.1/2C.2 route-level smoke does NOT depend on this candidate (the route runs inside the dev-server process, where `next/headers.cookies()` works normally). A future candidate slice would dependency-inject the Supabase client into the resolver/runtime where appropriate so standalone smoke/integration scripts can exercise the full orchestration path without faking a Next.js request context. Tracked as a candidate only — not approved, not in-flight.
