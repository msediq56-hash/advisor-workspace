...
Verification baseline is complete across all current runtime layers (305 tests across 19 test files). Milestone 0 (core runtime + evaluators + trace renderers) is complete. Milestone 1B (catalog dual-ownership RLS for universities, programs, program_offerings) is complete and runtime-validated. Milestone 1C (faculties child RLS following parent university visibility) is complete and runtime-validated. Broader RLS closeout is NOT complete yet — qualification tables, rule tables, student profile tables, evaluation runtime tables, and governance tables still lack RLS. Next recommended step: Milestone 1D — qualification RLS: (1) `qualification_types` using `is_visible_by_ownership`; (2) then child policies for `qualification_question_sets`, `qualification_questions`, and `qualification_question_options`.
