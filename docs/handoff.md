...
- `src/types/direct-evaluation-trace-explanation.ts` — trace-level explanation input/result types (includes optional `matchedSubjectName`, `requiredSubjectNames`, `matchedGradeValue`, `requiredMinimumGradeValue` for subject-based evaluator support)
- `src/modules/evaluation/render-direct-evaluation-rule-trace-explanation.ts` — dedicated pure Arabic trace explanation renderer (`minimum_subject_count`, `required_subject_exists`, and `minimum_subject_grade`; passed/failed/skipped; throws on unsupported types or missing required data)
...
- `src/modules/evaluation/run-and-persist-direct-evaluation.ts` — service-layer composition: generic runtime + trace explanation renderer + persistence write service; caller-owned metadata explicit; `minimum_subject_count`, `required_subject_exists`, and `minimum_subject_grade` use dedicated renderer; unsupported trace explanation rule types use fixed compatibility fallback regardless of outcome; no UI, no routes, no API handlers, no server actions
...
- `src/modules/evaluation/run-and-persist-direct-evaluation.test.ts` — 15 workflow tests (delegation, metadata passthrough, trace explanation sourcing for minimum_subject_count, required_subject_exists, and minimum_subject_grade passed/failed, unsupported non-skipped compatibility fallback, unsupported skipped compatibility, null rule set, failure passthrough)
...
- `src/modules/evaluation/execute-direct-evaluation-rule-context.test.ts` — 20 execution engine tests (supported British pass/fail for minimum_subject_count, required_subject_exists, and minimum_subject_grade, non-British skip for all three, unsupported type skip, group outcome derivation for all_required and any_of modes, empty groups, multiple groups, output structure)
- `src/modules/evaluation/render-direct-evaluation-rule-trace-explanation.test.ts` — 6 trace renderer tests (minimum_subject_grade passed/failed-below-threshold/failed-no-match/skipped, two-failure-shape distinction, unsupported type throw)
...
- No business UI
- No broader evaluator support beyond the current narrow subject-based baseline (`minimum_subject_count`, `required_subject_exists`, `minimum_subject_grade`)
- No broader dedicated trace explanation rendering beyond `minimum_subject_count`, `required_subject_exists`, and `minimum_subject_grade` (unsupported trace explanation rule types use a fixed compatibility fallback in workflow regardless of outcome)
...
Verification baseline is complete across all current runtime layers (189 tests across 15 test files). Four runtime repairs, two post-repair hardening micro-slices, and the `minimum_subject_grade` evaluator + trace renderer slices have been applied. Next narrow execution slice: extend evaluator support with the next rule type baseline, or begin broader integration/golden-case verification.
